...
from .const import DOMAIN, DEFAULT_USER, DEFAULT_HOST, CONF_CLIENT_CLASS, CONF_SCAN_RETRIES, CONF_SCAN_BACKOFF
        ...
        schema = vol.Schema(
            {
                ...
                vol.Optional(CONF_SCAN_RETRIES, default=3): cv.positive_int,
                ...
                schema = vol.Schema(
                    {
                        ...
                        vol.Optional(
                            CONF_SCAN_RETRIES,
                            default=user_input.get(CONF_SCAN_RETRIES, 3),
                        ): cv.positive_int,
                        vol.Optional(
                            CONF_SCAN_BACKOFF,
                            default=user_input.get(CONF_SCAN_BACKOFF, 1.0),
                        ): cv.positive_float,
                        vol.Required(
                            CONF_VERIFY_SSL,
...
class OptionsFlow(config_entries.OptionsFlowWithConfigEntry):
    ...
    async def async_step_init(self, user_input: dict[str, Any] | None = None) -> FlowResult:
        ...
        data_schema = vol.Schema({
            ...
            vol.Optional(CONF_SCAN_RETRIES, default=data.get(CONF_SCAN_RETRIES, 3)): cv.positive_int,
